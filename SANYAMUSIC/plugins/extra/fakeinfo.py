...
async def address(_, message):
    try:
        if len(message.command) > 1:
            ...
            query = "US"  # Default to US if no country is provided
...
﹝⌬﹞**ᴘʜᴏɴᴇ** ⇢ {phone}
        ...
        await message.reply_text(f"API ʀᴇᴛᴜʀɴᴇᴅ ᴀɴ ᴇʀʀᴏʀ ғᴏʀ ᴄᴏᴜɴᴛʀʏ ᴄᴏᴅᴇ {query}. Pʟᴇᴀsᴇ ᴛʀʏ ᴀɴᴏᴛʜᴇʀ ᴏɴᴇ (ᴇ.ɢ., US, IN, GB).")
    except Exception as e:
        await message.reply_text(f"Aɴ ᴇʀʀᴏʀ ᴏᴄᴄᴜʀʀᴇᴅ: {e}")