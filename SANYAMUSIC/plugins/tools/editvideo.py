...
async def remove_media(_, message: Message):
    ...
    file_path = None
    try:
        file_path = await replied.download(file_name=f"media_input_{message.id}.mp4")

        if command == "audio":
            output_audio = f"output_audio_{message.id}.mp3"
            ...
            output_video = f"output_video_{message.id}.mp4"
